...
- (void)yb_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
        ...
        if (status) {
//            [subAtt addAttribute:NSBackgroundColorAttributeName value:kBackgroundColor range:NSMakeRange(0, subAtt.string.length)];
...
- (void)setLineBreakByTruncatingLastLine:(NSString *)lastText
    ...
    NSArray *separatedLines = [self getSeparatedLinesArrayWithContentWidth:width text:contentText];
    if (separatedLines.count >= self.numberOfLines) {
        CGFloat wid1 = [MIHelpTool measureSingleLineStringWidthWithString:[NSString stringWithFormat:@"...%@", lastText] font:self.font];
        NSString *lastStr = separatedLines[self.numberOfLines - 1];
        CGFloat wid2 = [MIHelpTool measureSingleLineStringWidthWithString:lastStr font:self.font];
        if (wid2 > width - wid1) {
            NSMutableString *str = [NSMutableString stringWithCapacity:0];
            for (NSInteger i = 0; i < self.numberOfLines; i++) {
                if (i == self.numberOfLines - 1) {
                    [str appendString:[NSString stringWithFormat:@"%@...%@", [lastStr substringToIndex:lastStr.length - 3 - lastText.length], lastText]];
                } else {
                    [str appendString:separatedLines[i]];
                }
            }
            ...
            NSMutableString *str = [NSMutableString stringWithCapacity:0];
            for (NSInteger i = 0; i < separatedLines.count; i++) {
                [str appendString:separatedLines[i]];
            }
            [str appendString:lastText];
...
- (NSArray *)getSeparatedLinesArrayWithContentWidth:(CGFloat)width text:(NSString *)text {
