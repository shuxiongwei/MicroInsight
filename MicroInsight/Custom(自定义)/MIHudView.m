...
{
    MIHudView *alert = [[MIHudView alloc] initWithMsg:msg];
//    if (!theView){
//        [[self getUnhiddenFrontWindowOfApplication] addSubview:alert];
//    }
//    else{
        [[MIHudView getWindow] addSubview:alert];
//    }
    [alert showAlert];
...
+ (void)showMsg:(NSString *)msg {
    return [self showMsg:msg inView:nil];
}

-(void)showAlert
{
    self.backgroundColor = UIColorFromRGBWithAlpha(0x333333, 1);
    self.alpha = 0.0;
    CGPoint center = [MIHudView getWindow].center;
    //    //调整位置
    //    center.y -= (int)((ScreenHeight - self.frame.size.height) / 164.0f * 36 / 2);
    self.center=center;
...
-(id)initWithMsg:(NSString*)_msg
{
    if (self = [super init]) {
        ...
        if (_msg.length>30) {
            _msg = [NSString stringWithFormat:@"%@..",[_msg substringToIndex:30]];
        }
        ...
        msgFont = [UIFont systemFontOfSize:12.0f];
        ...
        if ([self getSizeFromString:_msg].height > 20) {
//            [labelText setFrame:CGRectMake((200 - textSize.width) / 2, 10,textSize.width, textSize.height)];
            self.bounds = CGRectMake(0, 0, textSize.width + 20, 10+textSize.height+10);
        }else {
            self.bounds = CGRectMake(0, 0, textSize.width + 20, 30);
        }
        [labelText setFrame:self.bounds];
...
-(CGSize)getSizeFromString:(NSString*)_theString
{
    ...
    CGSize tempSize = [_theString boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attributes context:nil].size;
