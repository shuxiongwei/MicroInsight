...
typedef void(^doneBlock)(NSDate *date, NSInteger hour, NSInteger minute);

@interface MIDatePickerView() <UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate>
{
    //日期存储数组
...
/**
 默认滚动到当前时间
 */
- (instancetype)initWithDateStyle:(WSDateStyle)datePickerStyle CompleteBlock:(void(^)(NSDate *date, NSInteger hour, NSInteger minute))completeBlock {
    self = [super init];
    if (self) {
        ...
        if (completeBlock) {
            self.doneBlock = ^(NSDate *date, NSInteger hour, NSInteger minute) {
                completeBlock(date, hour, minute);
            };
        }
    }
    return self;
...
- (instancetype)initWithDateStyle:(WSDateStyle)datePickerStyle scrollToDate:(NSDate *)scrollToDate CompleteBlock:(void(^)(NSDate *))completeBlock {
    ...
    if (self) {
        ...
        if (completeBlock) {
            self.doneBlock = ^(NSDate *selectDate, NSInteger hour, NSInteger minute) {
...
- (IBAction)doneAction:(UIButton *)btn {
    ...
    self.doneBlock(_startDate, hourIndex, minuteIndex);
    [self dismiss];
}
