...
@interface MIHorizontalPickerView () <UIScrollViewDelegate>
...
// cell数组
@property (nonatomic, strong) NSMutableArray<MIHorizontalPickerViewCell *>* cells;
// cell的宽高
...
@implementation MIHorizontalPickerView
#pragma mark - 懒加载
- (NSMutableArray<MIHorizontalPickerViewCell *>*)cells
{
    if (!_cells) {
...
#pragma mark - 初始化
// 初始化方法
- (instancetype)initWithFrame:(CGRect)frame capacityOfDisplayCell:(NSInteger)capacity delegate:(id<MIHorizontalPickerViewDelegate>)delegate dataSource:(id<MIHorizontalPickerViewDataSource>)dataSource
{
...
// 初始化方法
+ (instancetype)horizontalPickerViewWithFrame:(CGRect)frame capacityOfDisplayCell:(NSInteger)capacity delegate:(id<MIHorizontalPickerViewDelegate>)delegate dataSource:(id<MIHorizontalPickerViewDataSource>)dataSource
{
...
- (void)layoutCells
{
    ...
    for (int i = 0; i < self.cells.count; i++) {
        // 获取cell
        MIHorizontalPickerViewCell* cell = self.cells[i];
        // 设置cell的frame
        CGFloat cellY = 0;
...
- (void)reloadData
{
    ...
    for (MIHorizontalPickerViewCell* cell in self.cells) {
    ...
    if ([self.dataSource respondsToSelector:@selector(horizontalPickerView:cellForColumnAtIndex:)]) {
        for (NSInteger i = 0; i < count; i++) {
            MIHorizontalPickerViewCell* cell = [self.dataSource horizontalPickerView:self cellForColumnAtIndex:i];
...
- (void)callDelegateMethodDidHighlightHorizontalPickerViewCell:(MIHorizontalPickerViewCell *)hpvc withIndex:(NSInteger)index
{
...
- (void)callDelegateMethodDidUnhighlightHorizontalPickerViewCell:(MIHorizontalPickerViewCell *)hpvc withIndex:(NSInteger)index
{
