...
@class UICustomCarouselModel;

/* 自定义轮播视图 */
@interface UICustomCarouselView : UIView

/**
 *  创建WMBannerView的+方法
 *
 *  @param frame                    设置的frame
 *  @param second                   设置自动滚动的秒数second(默认2s)
 *  @param modelsArray              一个包含WMBannerModel的数组
 *  @param placeholderImageName     本地占位图片名
 ...
 *  @param modelsArray              一个包含WMBannerModel的数组
 *  @param placeholderImageName     本地占位图片名
 *  @param imageViewContentMode     图片的内容模式 UIViewContentMode
 *  @param clickedCallBack          点击后的回调
 *  @param scrolledCallBack         视图滚动后回调
 *
 *  @return 返回WMBannerView的对象
