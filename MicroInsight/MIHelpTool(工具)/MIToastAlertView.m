...
@implementation MIToastAlertView
...
+ (void)showAlertViewWithMessage:(NSString *)message {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([v isKindOfClass:[MIToastAlertView class]]) {
            [v removeFromSuperview];
        }
        
        MIToastAlertView *alert = [[MIToastAlertView alloc] initWithFrame:MIScreenBounds message:message];
...
- (void)configToastAlertUI {
    ...
    CGFloat second = 2;
    CGFloat imgWidth = 12;
    CGFloat messageWidth = [MIHelpTool measureSingleLineStringWidthWithString:_message font:[UIFont systemFontOfSize:15]];
    ...
    CGRect rect = CGRectMake((MIScreenWidth - alertWidth) / 2.0, -alertHeight, alertWidth, alertHeight);
    ...
    UILabel *titleLab = [MIUIFactory createLabelWithCenter:CGPointZero withBounds:CGRectZero withText:_message withFont:15 withTextColor:UIColorFromRGBWithAlpha(0x00102C, 1) withTextAlignment:NSTextAlignmentLeft];
    ...
    [UIView animateWithDuration:0.5 delay:0 usingSpringWithDamping:0.4 initialSpringVelocity:1 options:UIViewAnimationOptionTransitionNone animations:^{
        ...
        [MIUIFactory addShadowToView:alertView withOpacity:1 shadowColor:UIColorFromRGBWithAlpha(0x000000, 0.12) shadowRadius:23 andCornerRadius:3];
