...
@property(nonatomic, strong) MICameraView *cameraView;
@property(nonatomic, strong) MIMotionManager *motionManager;
...
@implementation MIPhotographyViewController

#pragma mark - 生命周期
- (instancetype)init {
    if (self = [super init]) {
        NSString *assetPath = [MIHelpTool assetsPath];
        _movieURL = [NSURL fileURLWithPath:[NSString stringWithFormat:@"%@/%@", assetPath, @"movie.mov"]];
        _motionManager = [[MIMotionManager alloc] init];
...
- (BOOL)setupAssetWriterAudioInput:(CMFormatDescriptionRef)currentFormatDescription {
    ...
    if ([_movieWriter canApplyOutputSettings:audioCompressionSettings forMediaType: AVMediaTypeAudio]) {
        ...
        if ([_movieWriter canAddInput:_movieAudioInput]) {
            ...
            [MIToastAlertView showAlertViewWithMessage:_movieWriter.error.localizedDescription];
            return NO;
        }
    } else {
        [MIToastAlertView showAlertViewWithMessage:_movieWriter.error.localizedDescription];
...
//曝光
- (void)exposAction:(MICameraView *)cameraView point:(CGPoint)point succ:(void (^)(void))succ fail:(void (^)(NSError *))fail {
    id error = [self exposeAtPoint:point];
    error?!fail?:fail(error):!succ?:succ();
}

//自动聚焦、曝光
- (void)autoFocusAndExposureAction:(MICameraView *)cameraView succ:(void (^)(void))succ fail:(void (^)(NSError *))fail {
    id error = [self resetFocusAndExposureModes];
    error?!fail?:fail(error):!succ?:succ();
}

//转换摄像头
- (void)swicthCameraAction:(MICameraView *)cameraView succ:(void (^)(void))succ fail:(void (^)(NSError *))fail {
    id error = [self switchCameras];
    error?!fail?:fail(error):!succ?:succ();
}

//闪光灯
-(void)flashLightAction:(MICameraView *)cameraView succ:(void (^)(void))succ fail:(void (^)(NSError *))fail {
    id error = [self changeFlash:[self flashMode] == AVCaptureFlashModeOn?AVCaptureFlashModeOff:AVCaptureFlashModeOn];
    error?!fail?:fail(error):!succ?:succ();
}

//手电筒
- (void)torchLightAction:(MICameraView *)cameraView succ:(void (^)(void))succ fail:(void (^)(NSError *))fail {
...
- (CGPoint)getDeviceMinAndMaxExposureIsoFactor:(MICameraView *)func {
    AVCaptureDevice *device = [self activeCamera];
