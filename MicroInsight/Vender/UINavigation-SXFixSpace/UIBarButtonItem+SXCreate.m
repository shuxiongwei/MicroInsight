...
+(UIBarButtonItem *)itemWithTarget:(id)target action:(SEL)action title:(NSString *)title {
    return [self itemWithTarget:target action:action title:title font:[UIFont systemFontOfSize:15] titleColor:nil highlightedColor:nil titleEdgeInsets:UIEdgeInsetsZero];
}
...
+(UIBarButtonItem *)itemWithTarget:(id)target
    ...
    [button sizeToFit];
    if (button.bounds.size.width < 50) {
        CGFloat width = 50 / button.bounds.size.height * button.bounds.size.width;
        button.bounds = CGRectMake(0, 0, width, 40);
    }
    if (button.bounds.size.height > 40) {
        CGFloat height = 50 / button.bounds.size.width * button.bounds.size.height;
        button.bounds = CGRectMake(0, 0, 50, height);
    }
