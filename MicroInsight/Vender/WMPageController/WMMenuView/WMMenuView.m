...
- (void)setFrame:(CGRect)frame {
    ...
    if (startX + contentWidth / 2 != self.bounds.size.width / 2) {
        ...
        if (self.topLineView) {
            self.topLineView.frame = CGRectMake(0, 0, self.frame.size.width, 0.5);
        }
    }
}
...
- (void)addScrollView {
    ...
    UIView *topLineView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, 0.5)];
    topLineView.backgroundColor = UIColorFromRGBWithAlpha(0xDDDDDD, 1);
    [self addSubview:topLineView];
    self.topLineView = topLineView;
    
    UIView *lineView = [[UIView alloc] init];
    lineView.backgroundColor = UIColorFromRGBWithAlpha(0xDDDDDD, 1);;
