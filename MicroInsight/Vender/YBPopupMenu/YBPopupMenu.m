...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _separatorColor = UIColorFromRGBWithAlpha(0xECECEC, 1);//[UIColor lightGrayColor];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UILabel *titleLab = [UILabel labelWithTitle:_titles[indexPath.row] color:UIColorFromRGBWithAlpha(0x333333, 1) fontSize:15 alignment:NSTextAlignmentLeft];
    titleLab.numberOfLines = 1;
    
    if (_images.count == 0 || !_images) {
        titleLab.textAlignment = NSTextAlignmentCenter;
        titleLab.frame = CGRectMake(0, 0, self.width, self.itemHeight);
    }else {
        UIImage *img = [UIImage imageNamed:_images[indexPath.row]];
        UIImageView *imgView = [[UIImageView alloc] initWithImage:img];
        CGFloat hgt = 23 * img.size.height / img.size.width;
        imgView.frame = CGRectMake(28, (50-hgt)/2, 23, hgt);
        [cell.contentView addSubview:imgView];
        
        titleLab.frame = CGRectMake(imgView.right + 23, 0, self.width-20-28-23, self.itemHeight);
    }

    if ([self.delegate respondsToSelector:@selector(ybPopupMenu:menuLabel:setItemColorAtIndex:)]) {
        [self.delegate ybPopupMenu:self menuLabel:titleLab setItemColorAtIndex:indexPath.row];
    }
    
    [cell.contentView addSubview:titleLab];
    
    if (indexPath.row != _titles.count-1) {
        UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, self.itemHeight-1, self.width, 1)];
        line.backgroundColor = UIColorFromRGBWithAlpha(0xECECEC, 1);
...
//    }
//    cell.separatorColor = kNewLineColor;
...
- (void)setIsShowShadow:(BOOL)isShowShadow
{
    ...
    self.layer.shadowOpacity = isShowShadow ? 0.2 : 0;
    self.layer.shadowOffset = CGSizeMake(0, 0);
    self.layer.shadowRadius = isShowShadow ? 4.0 : 0;
    self.layer.shadowColor = UIColorFromRGBWithAlpha(0x636363, 1).CGColor;
}
